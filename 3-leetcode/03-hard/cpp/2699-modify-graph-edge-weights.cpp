...
public:
    ...
    long runDijkstra(const vector<vector<int>> &edges, int nodeCount,
                     int sourceNode, int destinationNode)
    {
        vector<vector<long>> adjMatrix(nodeCount,
                                       vector<long>(nodeCount, INF));
        vector<long> minDistance(nodeCount, INF);
