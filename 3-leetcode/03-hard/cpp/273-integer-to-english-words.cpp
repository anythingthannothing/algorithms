...
{
public:
    vector<pair<int, string>> numberToWordMap = {
    ...
    string numberToWords(int num)
    {
        ...
        for (auto &it : numberToWordMap)
        {
            if (num >= it.first)
            {
                string prefix = (num >= 100) ? numberToWords(num / it.first) + " " : "";

                string unit = it.second;

                string suffix = (num % it.first == 0) ? "" : " " + numberToWords(num % it.first);
